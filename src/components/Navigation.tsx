...
export default function Navigation() {
  ...
  return (
    <nav className="bg-primary text-white sticky top-0 z-50 shadow-lg">
      <div className="navbar-top-bar container mx-auto px-6 h-16 flex items-center justify-between gap-6">
        <Link to="/" className="bg-white p-2 rounded-lg shadow-sm hover:shadow-lg hover:scale-105 transition-all duration-200 flex-shrink-0">
          <img src="/images/logo.webp" alt="Vineyard Technologies" className="h-8 w-auto" />
        </Link>
        
        <div className="navbar-content">
          {/* Desktop Navigation */}
          <div className="navbar-links desktop-only hidden md:flex items-center space-x-8">
            <NavigationMenu>
              <NavigationMenuList>
                <NavigationMenuItem>
          ...
          {/* Mobile Hamburger */}
          <button 
            className="navbar-hamburger mobile-only md:hidden" 
      ...
      <ul className={`navbar-menu mobile-only ${mobileMenuOpen ? 'open' : ''}`} role="list">
        ...
        <li><Link to="/services/software-development" onClick={() => setMobileMenuOpen(false)}>Software Development</Link></li>
        <li><Link to="/services/development-outsourcing" onClick={() => setMobileMenuOpen(false)}>Development Outsourcing</Link></li>
        <li><Link to="/services/technical-consulting" onClick={() => setMobileMenuOpen(false)}>Technical Consulting</Link></li>
        <li><Link to="/products/overlord" onClick={() => setMobileMenuOpen(false)}>Overlord</Link></li>
        <li><Link to="/products/daggerquest" onClick={() => setMobileMenuOpen(false)}>DaggerQuest</Link></li>
        <li><Link to="/products/mythic-market-mogul" onClick={() => setMobileMenuOpen(false)}>Mythic Market Mogul</Link></li>
